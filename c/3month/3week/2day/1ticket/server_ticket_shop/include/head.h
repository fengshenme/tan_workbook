...
int sqllogin(sqlite3 *ppdb, u_node *unode);  //登录验证
int sqlrepass(sqlite3 *db, u_node *unode); //重置密码

void searchall(); // 查找全部用户
int search_fli_all(sqlite3 *ppdb, f_list flist); // 查找所有航班

void show_list(u_list ulist); // 显示所有用户信息,并清空列表
