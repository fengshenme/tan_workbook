...
int login(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        ...
        if (sql_login(c_list->pdb, utmp) == 0)
        {
            flag = 1;
            utmp->cmd = 200;
            // sprintf(lsmg, "200:%s,%s,%hd,%d,%u", utmp->name, utmp->phone, utmp->status, utmp->account, utmp->vipclass);
            // printf("[%d] %s\n", __LINE__, lsmg);
            if (send(c_list->confd, utmp, sizeof(u_node), 0) < 0) //传入的链表不是头节点,而是使用节点,其confd有当前的连接id
            {
                flag = -1;
...
int search_flight(char *buf, cp_list c_list)
{
    ...
    sql_find_fli_all(c_list->pdb, flist); //从数据库中加载数据到链表
    ...
    list_for_each_entry(ftmp, &flist->head, head)
    {
        ...
        if (strncmp(des, ftmp->destin, strlen(ftmp->destin)) == 0 && strncmp(botime, ftmp->Address, strlen(ftmp->Address)) == 0 
                    && ftype[0] == ftmp->ftype && ftmp->price < atoi(price))
        {
            bzero(lsmg, 256);
            sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                    ftmp->destin, ftmp->Address, ftmp->ftype, ftmp->boardingTime, ftmp->price, ftmp->surplusVote);
...
int recharge(char *buf, cp_list c_list)
{
    ...
    u_list utmp = malloc(sizeof(u_node));
    ...
    if (flag != -1)
    {
        ...
        if (sql_recharge(utmp, c_list->pdb) == 0)
        {
...
int balance_inquiry(char *buf, cp_list c_list)
{
    ...
    int slen = 256;
    if (flag != -1 && sql_balance_inquiry(c_list->pdb, utmp) == 0)
    {
...
int s_booking_flights(char *buf, cp_list c_list)
{
    ...
    if(sql_find_flght_one(c_list->pdb, ftmp) == 0)
    {
        if(ftmp->surplusVote <= 0)
            printf("%d 票已售完\n", __LINE__);
    }

    
    if (sql_buy_flight(c_list->pdb, btmp) == 0)
    {
