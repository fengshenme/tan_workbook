...
static int f_tmp = 0; // 重新加载函数的状态码

static int img_i = 0; //显示图片的下标

#define LEFT2RIGHT_IN 2

#define RIGHT2LEFT_IN 3
...
int view_init(char (*pic_list)[256], int pic_size)
{
    ...
    while (1)
    {
        ...
        int max_fd = ts_fd > STDIN_FILENO ? ts_fd : STDIN_FILENO;
        ...
        { //shell指令更换上下图片
            ...
            {
                img_i++;
                effect = LEFT2RIGHT_IN;
                jpg_trans(pic_list[img_i % pic_size], effect);
            }
            if (strncmp(buf, "prev", 4) == 0)
            {
                img_i--;
                img_i = img_i < 0 ? pic_size : img_i;
                effect = RIGHT2LEFT_IN;
                jpg_trans(pic_list[img_i % pic_size], effect);
            }
        }
        ...
        if (FD_ISSET(ts_fd, &set)) //触摸切换
        {
            printf("from client:%s", "65 line dodsalkslk \n");
...
int jpg_trans(char *filepath, int effect)
{
    ...
    show_img(bmp_buffer, &imageinfo, effect);
    return 0;
}
