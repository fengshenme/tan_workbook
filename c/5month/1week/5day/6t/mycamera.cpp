...
int MyCamera::camera_capture()
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        //把转换得到的ARGB数据填充液晶屏
        for (j = 0; j < H; j++)
            memcpy(lcdmem + 800 * j + 480, &argbbuf[W * j], W * 4);
    }
    return 0;
}
...
int MyCamera::save_pic()
{
    ...
    QString str = QString("/root/repodemo/image/stopimg/%1%2%3")
                      .arg(date.toString("yyyy_MM_dd_"))
                      .arg(time.toString("hh_mm_ss_zzz"))
                      .arg(".jpg");
