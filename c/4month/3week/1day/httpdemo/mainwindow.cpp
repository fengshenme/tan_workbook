...
void MainWindow::fun(QNetworkReply* reply)
{
    ...
    //再解析
    QJsonValue city=obj.value("city");
    QJsonObject cityobj=city.toObject();
    qDebug() << city.toString();

    qDebug() << otherobj.find("city").value().toString();
    qDebug() << otherobj.find("temp1").value().toString();
    qDebug() << otherobj.find("temp2").value().toString();
