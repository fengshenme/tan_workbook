...
import org.web.demo.mapper.sql.DeleteSql;
import org.web.demo.mapper.sql.InsertSql;
import org.web.demo.mapper.sql.SelectSql;
import org.web.demo.mapper.sql.UpdateSql;
...
public interface BaseMapper<T> {
	...
	List<T> selectAll(@Param(value = "entityName") String entityName, @Param(value = "index") Integer index,
	...
	/**
	 * 根据字段查询,entityName字符串通过','拆分成数组 <br>
	 * 数组索引0是实体类名,1则是字段
	 * 
	 * @param entityName
	 * @param value
	...
	List<T> selectByPageTotal(@Param(value = "entityName") String entityName,
			@Param(value = "values") List<String> values);

	/**
	 * @param entityName 实体类名,以及条件字段 分页条件查询,不返回总记录数,但提升查询速度<br>
	...
	int insertAll(@Param(value = "entityName") String entityName,
			@Param(value = "recordList") List<? extends T> recordList);
	...
	boolean updateByPrimaryKey(@Param(value = "entityName") String entityName,
			@Param(value = "filed") List<String> filed);
