	    	...
	    	if(input.isFile()) {
	    		...
	    		try {
		    		...
		    		for (Element elem : selects) {
						...
						String friendinfo = sel.select("dd.mod_info.S_line1").text();
						String sex = sel.select("dd.mod_info.S_line1").select("div.info_name.W_fb.W_f14").select("i.W_icon").attr("class");
						String nickname = sel.select("dd.mod_info.S_line1").select("div.info_name.W_fb.W_f14").select("a").text();
						String followfans = sel.select("dd.mod_info.S_line1").select("div.info_connect").text();
						weiboFollow.setId(IdUtil.getSnowflake(1, 1).nextIdStr());
						...
						logger.info(weiboFollow.toString());
