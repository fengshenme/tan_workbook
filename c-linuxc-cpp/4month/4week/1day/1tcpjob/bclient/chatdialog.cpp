...
ChatDialog::ChatDialog(QWidget *parent, QString ipa, QTcpSocket *cli): QDialog(parent)
{
    ...
    chatlistwid->setStyleSheet("border-radius:20px; background-color: rgb(220, 220, 220); ");
//    chatlistwid->setStyleSheet("ChatDialog{border-radius:20px; background-color: rgb(220, 220, 220); border:1px solid#BFBFBF;}");
...
//          QHBoxLayout *layout=new QHBoxLayout;
            QLabel *tit = new QLabel(w);
            QLabel *lab = new QLabel(w);
//            layout->addWidget(tit);
//            layout->addWidget(lab);
//            w->setLayout(layout);
            tit->setGeometry(0, 0, 50, 200);
            tit->setText(sta.section(":", 1, 1));


            QPixmap pap;
            pap.loadFromData(byar);
            lab->setPixmap(pap.scaledToWidth(200));
            lab->setGeometry(50, 0, 200, 200);

            item->setSizeHint(QSize(size.width(), 200));

            chatlistwid->addItem(item);

            w->setSizeIncrement(size.width(), 200);
            chatlistwid->setItemWidget(item, w);

        }
        else if(msga.section(':', 0, 0) == "m")
        {
            chatlistwid->addItem(QString(msg).section(":", 2));
        }
...
{
    //|窗口关闭之前需要的操作~
    this->parentWidget()->update();// ->setCheckState(Qt::Checked);
}
