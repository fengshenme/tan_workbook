...
int main(int argc, char **argv)
{
    ...
    unsigned char rgb_buffer[800*480*3] = {0};
    int i, j;
    for ( i = 0, j=0; j < 800*480; i+=3, j++)
    {
    //   rgb_buffer[i] =  argb_sum[j] >> 24;
      rgb_buffer[i] = 0x00 | argb_sum[j] >> 16;
      rgb_buffer[i+1] = 0x00 | argb_sum[j] >> 8;
      rgb_buffer[i+2] = 0x00 | argb_sum[j] ;
    }
    
    char des[30];
    sprintf(des,"a%s", argv[1]);
   if( write_jpeg_file(des, rgb_buffer, 800, 480, 90) == 0) //宽800, 高480 压缩率90
     printf("success \n");
