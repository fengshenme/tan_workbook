...
void write_lcd_t(unsigned long *image, unsigned char *bmp_buffer, struct image_info *imageinfo,
...
int main(int argc, char **argv)
{
	...
	unsigned long(*img)[WIDTH] = calloc(SCREEN_SIZE, 4);
	printf("%lx \n", image[200] );
	int w, h, i = 0;
	for (h = 0; h < HEIGHT; h++)
	{
	...
	a_falling_down_in(FB1, img);

	memcpy(FB1, image, SCREEN_SIZE * 4);
				 ...
				 unsigned char *FB, struct fb_var_screeninfo *vinfo)
{
	...
	for (x = 0; x < vinfo->yres && x < imageinfo->height; x++)
	{
		for (y = 0; y < vinfo->xres && y < imageinfo->width; y++)
		{
			...
			*(image+i) = (tmpc[0] << 24 | tmpc[1] << 16 | tmpc[2] << 8 | tmpc[3] << 0);
