...
void *p_copy_file(void *arg)
{
    ...
    plural_path *pd_path = malloc(sizeof(plural_path));

    memset(pd_path, 0, sizeof(plural_path));
    memcpy(pd_path, arg, sizeof(plural_path));

    // strcpy(pd_path->d_path, p_path->d_path);
    // strcpy(pd_path->s_path, p_path->s_path);

    // pd_path->pool = p_path->pool;
...
// void *pfiletofile(char *src_path, char *des_path, unsigned int perm)
{
    ...
    printf("is file \n");
    fr = open(pd_path->s_path, O_RDONLY, pd_path->perm);
    fw = open(pd_path->d_path, O_WRONLY | O_CREAT | O_TRUNC, pd_path->perm); //没有就创建,有就清空在写入
    if (fr == -1)
    {
        fprintf(stderr, "file open failed \n");
    }
