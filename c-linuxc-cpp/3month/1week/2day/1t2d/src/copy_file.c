...
void show()
{
    printf("copied file_total: %d \n", count);
    printf("copied dir_total: %d \n", dir_count);
}

void *p_copy_file(void *arg)
{

    plural_path *pd_path = (plural_path *)arg;
    ...
    {
        fprintf(stderr, "[%d]: file not exist or get file info failed\n", __LINE__);
        exit(1);
    }
    ...
    if ((fileinfo->st_mode & S_IFMT) == S_IFDIR && access(des_path, F_OK) != 0) // 如果源文件为目录文件,目标目录文件不存在,则创建目录文件
    {
    ...
    {
        struct stat *dirfile_info = malloc(sizeof(struct stat));
        ...
        else if ((fileinfo->st_mode & S_IFMT) == S_IFDIR && (dirfile_info->st_mode & S_IFMT) == S_IFREG)
        {
            fprintf(stderr, "The directory cannot copy to a non-directory! \n");
            exit(1);
        }
        else if ((fileinfo->st_mode & S_IFMT) == S_IFDIR && (dirfile_info->st_mode & S_IFMT) == S_IFDIR)
        {
            ...
            // 源目录与目标目录检测
            if (strncmp(src_path, des_path, strlen(src_path)) == 0)
            {
            ...
            read_dir(src_path, des_path, pd_path);
        }
        free(dirfile_info);
    }
