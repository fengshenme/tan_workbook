...
int main(int argc, char const *argv[]) // /.server.out 端口号
{
    ...
    if ((pthread_create(&r_pid, NULL, read_msg, (void *)c_list)) != 0)
    {
        printf("get pthread create failed!\n");
    }
    if ((pthread_create(&s_pid, NULL, send_msg, (void *)c_list)) != 0)
    {
        printf("get pthread create failed!\n");
    }
...
void *send_msg(void *arg)
{
    ...
    while (1)
    {
        ...
        list_for_each_entry_safe(pos, n, &c_list->head, head)
        {
            ...
            if (strlen(pos->msg) > 0)
            {
                char *p = strstr(pos->msg, ":");
                if (p == NULL)
                {

                    list_for_each_entry(tmp, &c_list->head, head)
                    {
                        // printf("line:[%d] from client  :%s", __LINE__, tmp->msg);
                        send(tmp->confd, pos->msg, sizeof(pos->msg), 0);
                    }
                }
                else
                {
                    int fd = atoi(pos->msg);
                    send(fd, p + 1, sizeof(pos->msg), 0);
                }
                bzero(pos->msg, sizeof(pos->msg));
            }
        }
    }
