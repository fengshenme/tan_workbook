...
int main(int argc, char const *argv[]) // /.server.out 端口号
{
    ...
    while (1)
    {
        ...
        if (concetfd > 0)
        {
            ...
            if ((pthread_create(&s_pid, &attr, read_msg, (void *)c_list)) != 0)
            {
...
void *read_msg(void *arg)
{
    ...
    pos = list_entry((c_list->head).next, c_node, head);
    printf("113, %d\n", pos->confd);
    // 5）畅聊
    char buf[200];
    while (1)
    {
        ...
        if (p == NULL)
        {
            ...
            {
                if (tmp->confd != pos->confd)
                {
                    send(tmp->confd, buf, sizeof(buf), 0);
                }
            }
        }
        else
        {
            char t_buf[360] = {0};
            int ctfd = atoi(buf);
            sprintf(t_buf, " %d:%s", ctfd, p + 1);
            send(ctfd, t_buf, sizeof(buf), 0);
        }
