...
void *read_msg(void *arg) // 线程例程函数
{
    ...
    while (1)
    {
        ...
        if (strncmp(buf, "getflight_list", 14) == 0)
            flight_list(buf, pos); // 获取航班列表
        if (strncmp(buf, "selectf", 7) == 0)
            search_flight(buf, pos); //多条件search 航班
        if (strncmp(buf, "selectq", 7) == 0)
            search_quick(buf, pos); // 通过航班号快速查找
        if (strncmp(buf, "recharge", 8) == 0)
            recharge(buf, pos); // 账户充值
        if (strncmp(buf, "buyflight", 9) == 0)
            s_booking_flights(buf, pos); // 订购航班
        if (strncmp(buf, "findacc", 7) == 0)
            balance_inquiry(buf, pos); // 余额查询
        if (strncmp(buf, "findbuyfli", 10) == 0)
            find_buyed_flight(buf, pos); // 已购航班
        if (strncmp(buf, "refunded", 8) == 0)
            refunded(buf, pos); // 退票
        if (strncmp(buf, "reuserin", 8) == 0)
            change_userinfo(buf, pos); // 修改用户信息
        if (strncmp(buf, "drawbill", 8) == 0)
            draw_bill(buf, pos); // 出票
        if (strncmp(buf, "pinvoice", 8) == 0)
            print_invoice(buf, pos); // 出票
    }
...
int login(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        ...
        {
            if (strncmp(name, utmp->name, strlen(name)) == 0 && strncmp(password, utmp->password, strlen(utmp->password)) == 0)
            {
                printf("[%d]%s,%s\n", __LINE__, password, utmp->password);
                char tapath[128];
                bzero(tapath, 128);
...
int regist_s(char *buf, cp_list c_list)
{
    ...
    if (code == 1212 && flag == 0)
    {
        ...
        sprintf(buf, "%s,%s,%s,%hd,%d,%u\n", unode->name, unode->password, unode->phone, unode->status, unode->account, unode->vipclass);
...
int repassword(char *buf, cp_list c_list)
{
    ...
    if (flag != -1 && atoi(code) == 1212)
    {
        list_for_each_entry(utmp, &ulist->head, head)
        {
            if (strncmp(name, utmp->name, strlen(name)) == 0 && strncmp(phone, utmp->phone, strlen(phone)) == 0)
            {
                char tapath[128];
                bzero(tapath, 128);
                sprintf(tapath, "data/user_db%d.txt", urand(3));
...
int flight_list(char *buf, cp_list c_list)
{
    ...
    list_for_each_entry(ftmp, &flist->head, head)
    {
        ...
        sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                ftmp->destin, ftmp->Address, ftmp->type, ftmp->boardingTime, ftmp->price, ftmp->surplusVote);
...
int search_flight(char *buf, cp_list c_list)
{
    ...
    {
        flag = -1;
        fprintf(stderr, "%d-] str strtok failed\n", __LINE__);
    }
    ...
    list_for_each_entry(ftmp, &flist->head, head)
    {
        ...
        if (strncmp(des, ftmp->destin, strlen(des)) == 0 && strncmp(botime, ftmp->Address, strlen(botime)) == 0 && *ftype == ftmp->type && ftmp->price < atoi(price))
        {
            bzero(lsmg, 256);
            sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                    ftmp->destin, ftmp->Address, ftmp->type, ftmp->boardingTime, ftmp->price, ftmp->surplusVote);
    ...
    {
        flag = -1;
        fprintf(stderr, " %d->msg send failed\n", __LINE__);
    }
...
int search_quick(char *buf, cp_list c_list)
{
    ...
    {
        flag = -1;
        fprintf(stderr, "%d=> str strtok failed => %s\n", __LINE__, p + 1);
    }
    ...
    list_for_each_entry(ftmp, &flist->head, head)
    {
        ...
        if (strncmp(fln, ftmp->flight_number, strlen(fln)) == 0)
        {
            bzero(lsmg, 256);
            sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                    ftmp->destin, ftmp->Address, ftmp->type, ftmp->boardingTime, ftmp->price, ftmp->surplusVote);
...
int recharge(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        list_for_each_entry(utmp, &ulist->head, head)
        {
            if (strncmp(phone, utmp->phone, strlen(phone)) == 0 && strncmp(name, utmp->name, strlen(name)) == 0)
            {
                char tapath[128];
                bzero(tapath, 128);
                sprintf(tapath, "data/user_db%d.txt", urand(3));
...
int s_booking_flights(char *buf, cp_list c_list)
{
    ...
    {
        if (strncmp(btmp->flight_number, flnum, strlen(flnum)) == 0 && strncmp(btmp->phone, phone, strlen(phone)) == 0)
        {
            break;
            flag = -1;
        }
    ...
    if (flag != -1)
    {
        list_for_each_entry(ftmp, &flist->head, head)
        {
            ...
            if (strncmp(flnum, ftmp->flight_number, strlen(flnum)) == 0 && strncmp(fltime, ftmp->Address, strlen(fltime)) == 0 && ftmp->surplusVote > 0)
            {
...
int find_buyed_flight(char *buf, cp_list c_list)
{
    ...
    list_for_each_entry(btmp, &blist->head, head)
    {
        ...
        if (strncmp(phone, btmp->phone, strlen(phone)) == 0)
        {
            list_for_each_entry(ftmp, &flist->head, head)
            {
                ...
                if (strncmp(btmp->flight_number, ftmp->flight_number, strlen(btmp->flight_number)) == 0)
                {
                    bzero(lsmg, 256);
                    sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                            ftmp->destin, ftmp->Address, ftmp->type, ftmp->boardingTime, ftmp->price, btmp->flags);
...
int refunded(char *buf, cp_list c_list)
{
    ...
    if (flag == 1)
    {
        list_for_each_entry(ftmp, &flist->head, head)
        {
            ...
            if (strncmp(flnum, ftmp->flight_number, strlen(flnum)) == 0 && strncmp(fltime, ftmp->Address, strlen(fltime)) == 0)
            {
                pthread_mutex_lock(&blist->m); // lock
                list_del(&btmp->head);
                ...
                if (file_copy("data/user_db.txt", tapath) == 0)
                {
                    list_for_each_entry(utmp, &ulist->head, head) // 用户表的余额减少买票钱
                    {
                        if (strncmp(phone, utmp->phone, strlen(phone)) == 0)
                        {
                            // printf("%d=>%s\n", __LINE__, btmp->phone);
                            utmp->account += ftmp->price;
                        }
...
int change_userinfo(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        ...
        list_for_each_entry(utmp, &ulist->head, head)
        {
            if (strncmp(oldphone, utmp->phone, strlen(phone)) == 0 && strncmp(oldname, utmp->name, strlen(name)) == 0)
            {
                char tapath[128];
                bzero(tapath, 128);
                sprintf(tapath, "data/user_db%d.txt", urand(3));
...
int draw_bill(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        list_for_each_entry(ftmp, &flist->head, head)
        {
            if (strncmp(ftmp->flight_number, flnum, strlen(flnum)) == 0 && strncmp(ftmp->Address, fltime, strlen(fltime)) == 0)
            {
                break;
            }
        }
        list_for_each_entry(btmp, &blist->head, head)
        {
            if (strncmp(btmp->flight_number, flnum, strlen(flnum)) == 0 && strncmp(btmp->Address, fltime, strlen(fltime)) == 0)
            {
...
int print_invoice(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        ...
        list_for_each_entry(btmp, &blist->head, head)
        {
            ...
            if (strncmp(btmp->flight_number, flnum, sizeof(btmp->flight_number)) == 0 
                && strncmp(btmp->Address, fltime, sizeof(btmp->Address)) == 0
                && strncmp(btmp->phone, phone, sizeof(btmp->phone)) == 0)
            {
                char tapath[128];
                bzero(tapath, 128);
