...
void *read_msg(void *arg) // 线程例程函数
{
    ...
    while (1)
    {
        ...
        if (strncmp(buf, "buyflight", 9) == 0)
            s_booking_flights(buf, pos); // 账户充值
        if (strncmp(buf, "findacc", 7) == 0)
            balance_inquiry(buf, pos); // 账户充值
    }
...
int search_flight(char *buf, cp_list c_list)
{
    ...
    list_for_each_entry(ftmp, &flist->head, head)
    {
        ...
        if (strncmp(des, ftmp->destin, strlen(des)) == 0 &&
            strncmp(botime, ftmp->Address, strlen(botime)) == 0 &&
            *ftype == ftmp->type && ftmp->price < atoi(price))
        {
            bzero(lsmg, 256);
            sprintf(lsmg, "%s,%s,%s,%s,%c,%s,%u,%u\n", ftmp->flight_number, ftmp->Starting,
                    ftmp->destin, ftmp->Address, ftmp->type, ftmp->boardingTime, ftmp->price, ftmp->surplusVote);
...
int s_booking_flights(char *buf, cp_list c_list)
{
    ...
    if (flag != -1)
    {
        list_for_each_entry(ftmp, &flist->head, head)
        {
            ...
            if (strncmp(flnum, ftmp->flight_number, strlen(flnum)) == 0 &&
                strncmp(fltime, ftmp->Address, strlen(fltime)) == 0 && ftmp->surplusVote > 0)
            {
                ...
                sprintf(buf, "%s,%s,%s,%u\n", bnode->phone, bnode->flight_number, bnode->Address, bnode->price);
                ...
                if (file_copy("data/user_db.txt", tapath) == 0)
                {
                    list_for_each_entry(utmp, &ulist->head, head) // 用户表的余额减少买票钱
                    {
                        ...
                        printf("%d=>%s,%s\n", __LINE__, utmp->phone, bnode->phone);
